...
> **🟣 Update 2026-09-21 (partea a doua) — identificare imobil: fluxul „nu l-am găsit” + preț 298 pe ambele identificări.** Declanșator: E-260728-VWFTT nu apărea la căutare în portalul topografului (căutarea filtra doar în tabul deschis; reparat, caută în toate etapele). Pe drum: **12 identificări stăteau în „așteptare client” cu nota „nu s-a putut identifica, are credit”** — clientul vedea „așteptăm un răspuns de la tine”, nu primea nimic, creditul trăia doar în notă. Decizie Raul: când imobilul nu apare în e-Terra, topograful cere la OCPI **certificatul oficial** (Ordin ANCPI 16/2019: cod 2.7.8 după adresă 100 lei, 2.7.6 după proprietar 125 lei, ~10 zile lucrătoare); răspunsul e document în ambele cazuri (cu CF → extras; negativ → **certificatul E livrarea**, serviciu complet, **fără credit, fără extras gratuit, fără raport PDF**). Livrat: buton **„Nu am găsit — depun certificat la OCPI”** în portal, status nou `identification_pending_ocpi` („Identificare nereușită — certificat OCPI depus”, badge mov, rămâne în „În procesare”) pe toate listele albe + constraint (migrarea **182**), email automat clientului, termen +10 zile lucrătoare, istoric; **preț 298 lei pe ambele identificări** (migrările 182/183, pagini + FAQ); `ancpi_cost_ron` 125 pe proprietar. Echipa: ghid în Knowledge Center + **PDF de 6 pagini pentru Mircea și fete** (`admin/identificare-imobil-ghid.pdf`). Mircea NU are acces la `/admin/ghid` (rol colaborator) — primește PDF-ul pe WhatsApp. Cele două motive de negăsire (Mircea): neintabulat / intabulat pe cartea veche, neconvertită. ⚠️ API-ul colaboratorului taie la 200 comenzi (193 azi). [ghid echipă](admin/identificare-imobil-nereusita.md) · [design](plans/2026-09-21-identificare-imobil-nereusita-design.md) · changelog: [căutare](changelog/2026-09-21-colaborator-cautare-toate-etapele.md) · [analiză](changelog/2026-09-21-identificare-imobil-nereusita.md) · [livrare](changelog/2026-09-21-identificare-nereusita-livrat.md)

> **🟣 Update 2026-09-21 — documentero.ro este LIVE pentru Google.** După analiza competitorilor (SERP real `pws=0&udm=14`: eghiseul absent din top 20 pe naștere, locul 1 pe căsătorie/celibat), cele 4 pagini de serviciu au fost rescrise și extinse (900–1.200 → 1.800–2.450 cuvinte: „Pe scurt” citabil, cine poate cere, termen real din DB, `FAQPage`, tabele pe țări/instituții, recenzii pe act), ghidul „pierdut” la 1.272, plus 2 ghiduri noi (acte necesare + sectoarele București; procură din străinătate). Celibat = „adeverință privind statutul civil, Anexa 18” (H.G. 255/2024, fosta Anexa 9). `DOCUMENTERO_INDEXABLE = true`, sitemap (16 URL-uri) trimis în GSC (verificat pe sishuletz@gmail.com), indexare cerută pe 9 pagini — căsătoria indexată în ~20 min. GA4 propriu `G-ND6HB81QXF` (cont eGhiseul, env în Vercel, verificat pe prod). Linkuri spre documentero din eghiseul (5 pagini), CJO + ecazier (footer), avocat-tarta.ro (footer + 2 articole). Contact: formular propriu (act/țară/cod), program L–V 08:00–16:00, `[documentero]` în subiect; **adresa de contact e contact@eghiseul.ro** (decizie: fără Zoho separat până crește), Reply-To reparat (era eghiseul pe toate emailurile de brand — acum urmează `contactEmail`); 12 șabloane verificate + 7 emailuri de test în Inbox. Vercel/DNS verificate (www 308, HSTS, cron-uri, DMARC adăugat). 4 poze reale generate puse pe site (ușă, semnătură, certificat pe masă, cuplu). Rămân: pozele 5–11, ghidurile 1–2/săpt., urmărirea GSC. [dosar](documentero/README.md) · [analiza + plan](documentero/analiza-competitori-seo.md) · [lansare](documentero/lansare.md) · [changelog](changelog/README.md)

> **🟣 Update 2026-09-20 — documentero.ro: pagini legale, emailuri pe brand, flux curat.** Pagini legale proprii (T&C, confidențialitate, anulare, cookies) + `legalBaseUrl` documentero; toate cele 13 emailuri către client pe brandul comenzii (`brandForOrder`, `from: brand.emailFrom`, `repaintForBrand`); wizardul fără texte eghiseul (selfie, semnătură, transfer, auth, specimen, contract docx cu `{{SITE_NAME}}`/`{{TC_URL}}`); OG + logo email; linkuri dofollow din eghiseul; GSC token; Resend documentero verificat; `www` 308; comenzile de test șterse; reveal-ul reparat la navigarea client-side. Jaccard față de eghiseul ≈ 0. Rămân: TXT GSC (dacă Domain), Zoho primire, GA4, poze reale, textele finale la revizie, scoaterea `noindex`. [dosar](documentero/README.md) · [lansare](documentero/lansare.md)

> **🟣 Update 2026-09-19 (seara) — documentero.ro: pagini publice, brand complet, comanda de test.** Site-ul e live (DNS Vercel, HTTPS, favicon propriu, `noindex` până la lansare): acasă + 4 pagini de serviciu + ghiduri + despre + contact; header cu mega-meniu „Servicii” (fără cont, „Urmărește comanda” → `/comanda/status/`), footer cu ANPC, 9 recenzii Google reale cu poze (fără mențiuni eghiseul), animații light + ritm de spațiere, `SOCIAL_PROOF` 470. Comanda de test `E-260919-ADXE7` a trecut wizardul pe brandul documentero până în checkout; titlul paginilor `(order)` reparat (`generateMetadata` în layout). Rămân: Resend pe domeniu (fără MX/TXT), `www` 308, GSC/GA4, pagini legale proprii, OG + logo email, textele eghiseul din KYC/contract/specimen. [dosar](documentero/README.md) · [lansare](documentero/lansare.md) · [changelog](changelog/README.md)

> **🟣 Update 2026-09-19 — documentero.ro: al doilea brand, același deploy (fundația).** După ce stare civilă (~50% din venit, cele mai scumpe comenzi) a pierdut 90%+ din expuneri în august, pornim un site de nișă pe `documentero.ro`, de sine stătător (wizard + plată + status pe el, NU redirect spre eghiseul = doorway), servit de ACELAȘI repo/Vercel/DB/admin. Livrat azi: rutare pe host (`rewrites` cu `has: host` + `proxy.ts` pentru calea internă), paginile eghiseul mutate în grupul `(eghiseul)`, rutele partajate în `(order)` cu header ales per host, `src/lib/brand/` (brandul CERERII din host, brandul COMENZII din `orders.platform` — migrarea **181**), Stripe return URLs + emailul de confirmare + linkurile de reluare pe brandul comenzii, chip „Platformă” + badge în admin, `buildPageMetadata({ brand })`, robots + sitemap curatoriat, placeholder `noindex`. Build + 2.002 teste verzi; smoke local cu `Host: documentero.ro`. ⚠️ De aplicat pe registrul central: `supabase/registry/003_platform_documentero.sql`. Design: canvas Claude Design cu 3 direcții (A/B/C); Raul alege C, cu altă paletă (în lucru). [analiza](seo/2026-09-19-site-satelit-stare-civila.md) · [spec](technical/specs/multi-brand.md) · [changelog](changelog/2026-09-19-documentero-fundatia.md)
>
> **🟣 Update 2026-09-18 (partea a doua) — testul A→Z al lui Raul, 25 de puncte, toate livrate; comenzile de test șterse.** Trei tranșe azi: (1) P0/P1 din prima listă — KYC cere act **și** selfie, comanda cu IBAN se vede în admin, tipul solicitantului corect, dovada plății vizibilă; (2) P2 + wizard — pasul 1 pornește cu datele contului, pasul de acte sare pentru cont verificat, facturarea din profilurile salvate, istoric tradus fără dubluri; (3) punctele 19–25 — motivul solicitării (cele frecvente primele, dropdown în jos, eroare inline), **actul din cont la pasul 2** fără scanare, opțiunile cu urgența ca element principal, pasul KYC cu un singur rând, contractul fără scroll orizontal, **plata prin transfer bancar completă pe pagina comenzii cu upload de dovadă și fără cont**. Inspecție Codex: 3 runde pe cod (28 constatări reparate) + 5 runde pe planul tranșei a treia + o rundă pe codul ei (în curs). Admin: comenzile finalizate coboară sub cele vii în „Toate". Analiză abandonuri: 65% din neplătite pleacă de la pasul 2 fără să tasteze nimic; Resend n-are tracking pornit — [analiza](marketing/2026-09-18-analiza-abandonuri-si-emailuri.md). Changelog-uri: [inspecție + test A→Z](changelog/2026-09-18-inspectie-codex-si-testul-a-z.md), [P2 + wizard](changelog/2026-09-18-feedback-testare-p2-si-wizard.md), [19–25](changelog/2026-09-18-feedback-testare-19-25.md), [26–34](changelog/2026-09-18-feedback-testare-26-34.md), [decontări + Oblio](changelog/2026-09-18-decontari-taxa-anulare-oblio-retry.md), [admin sort](changelog/2026-09-18-admin-comenzi-finalizate-jos.md). (4) punctele 26–34 — un singur card din cont la pasul 2, reasigurare pliată, preț sub opțiune pe mobil, **easybox estimat real** (Sameday cerea județ + localitate și la locker; Odoreu 25,57 cu TVA față de 36,06 la adresă), facturare pe două niveluri, status curat pe `proof_verified_at`. Decontări: anulările fără factură inițială se leagă de factura de taxă (`po_1UGTTLHGb8JBHhclSJd80vUX`), Oblio reîncearcă la token expirat. Deschise: rezervarea cuponului la checkout, OCR/face-match server-side înainte de `kyc_verified`, facturare CJO la plată (nu la 06:00), tracking Resend.
>
> **🟣 Update 2026-09-18 — contul clientului, a doua zi: legături complete, meniu, cupon.** Zece livrări pe `/account`, toate din testul lui Raul pe telefon: telefonul cu selector de țară și validare pe țară în cont, popup-uri și înregistrare (`validate-phone.ts`, `PATCH /api/user/profile` refuză 400); **actul de identitate nu mai e pas în cont** (D9) — se cere doar în comandă, cu selfie, la serviciile care îl cer, iar poza scanată la „Date personale" se salvează în tabul de act; popup-ul de facturare pornește cu „le folosim și pe factură?" (D10); **„Datele mele" e meniul din vârful paginii** (rânduri cu ce e salvat / popup pentru ce lipsește; o linie „Profil complet 100%" la final), butoanele cu chenar; tabul deschis se reîncarcă după o salvare din popup (`account:data-saved`); întrebarea de onboarding scoasă (D12); **comanda plătită alimentează contul existent** — adresă, facturare, acte, la orice confirmare de plată + backlog la încărcarea contului (`sync-paid-order.ts`, D13); **cupon de bun-venit** BUNVENIT-… 10%/30 zile/o folosire, legat de cont (migrarea **174**, D11), purtat pe linkurile din catalog. 🔴 Două bug-uri reale prinse pe drum: linkurile din checklist erau moarte (`AccountTabs` citea `?tab=` doar la montare) și **pasul 1 al comenzii pierdea telefonul** când prefill-ul din cont sosea în timpul tastării (modul read-only pornea de la `state.contact` cu `length >= 8`) — comanda mergea mai departe fără cod, deci fără draft. Audit cont → comandă → admin verificat în browser și DB. Incident colateral 17.09: un loop de polling `npx vercel ls` fără token a deschis sute de taburi (CLI-ul pornește login-ul singur când detectează Claude Code) — regulă: fără polling cu CLI-ul Vercel. [ghid echipă](admin/contul-clientului.md) · changelog: [telefon](changelog/2026-09-18-telefon-cont-camp-comun.md) · [act la comandă + facturare](changelog/2026-09-18-act-doar-la-comanda-facturare-din-act.md) · [meniu + tab](changelog/2026-09-18-meniul-datelor-in-varful-contului.md) · [comanda → cont](changelog/2026-09-18-comanda-platita-alimenteaza-contul.md) · [meniu sus, fără întrebare](changelog/2026-09-18-cont-meniu-sus-fara-intrebare.md) · [cupon + pasul 1](changelog/2026-09-18-cupon-bun-venit-si-pasul-1.md)
>
> **🔴 Update 2026-09-14 (partea a treia) — „Nu știu" din Date imobil lăsa clientul fără comandă pe server.** Declanșator: `E-260914-B8SM9` („nu îl lasă să plătească"). Saltul „Nu știu" → Identificare imobil purta contactul, dar nu genera codul comenzii (se generează doar la ieșirea din pasul 1, sărit de handoff); auto-save-ul refuză fără cod → zero draft pe serviciul nou → „Plătește" răspundea la nesfârșit „Comanda se salvează încă…". Reprodus pe prod. Fix: `lib/orders/wizard-handoff.ts` (pur, testat) alocă codul. Plus UX din capturi: caseta ANCPI nu mai apare pe pasul de plată; butonul plutitor WhatsApp scos de pe wizard (stătea peste „Plătește"), înlocuit cu „Scrie-ne pe WhatsApp" în caseta de ajutor. Runda 2: **checkout-ul redus la esențial pe toate serviciile** (metoda de plată + cupon, apoi rezumat; fără caseta ANCPI, fără „Detalii Card", fără buton plutitor) și „Motiv solicitare" scos de pe cadastru. Detalii: [changelog](changelog/2026-09-14-nu-stiu-handoff-fara-draft.md).
>
> **🟣 Update 2026-09-14 — email marketing A–Z, livrat și PORNIT.** Declanșator: cuponul automat de recovery avea **1,4% redemption** (20 din 1.444), iar cele 72.278 contacte din registrul vechi nu primiseră niciodată un email. Într-o zi, 8 commituri: (1) **coadă de recuperare telefonică** `/admin/recuperare-telefonica` (telefon străin + stare civilă primele, apoi cele sub 24 h; un rând per email; „Bifează sunat" creează cuponul `TEL-` și trimite clientului emailul „ai vorbit cu <agent>, ai X% reducere, reia din link" cu cuponul auto-aplicat); (2) **warm-up** pe registrul de 72k, pornit la 25/zi; (3) **lifecycle** către clienți: recenzie Google doar la comenzi în termen, reminder expirare (cazier/integritate 6 luni; fiscal, auto, constatator, extras CF 30 zile), cross-sell la 30–60 zile — toate pornite; (4) **editor de campanii** (noutăți/articole, markdown-lite, test pe adresa proprie, tranșe zilnice) — nimic pornit, așteaptă text; (5) **recovery în 3 pași** (30 min fără cupon → +24 h încredere → +48 h cupon); (6) card **„KPI marketing"** (trimise/comenzi/venit per canal, UTM pe toate linkurile → `orders.attribution.last`). Review-ul codului scris de agent a prins 3 bug-uri de builder postgrest-js refolosit (regulă nouă în `.claude/rules/database.md`) și un filtru care ascundea 162 de comenzi din coadă (numele stă în `billing` la extras CF). Curățenie: 1.423 cupoane expirate ies din listă și se șterg automat, drafturile doar-contact >30 zile la fel. `orders.completed_at` nou (nu exista). Migrările **156–160** aplicate. Primele trimiteri automate: 15.09, 07:00–07:40 UTC. Ghidul echipei refăcut (v2, sursă HTML). [changelog](changelog/2026-09-14-recuperare-telefonica-warmup-email.md) · [lifecycle + KPI](technical/specs/lifecycle-emails.md) · [recuperare telefonică](technical/specs/phone-recovery-abandoned-carts.md) · [warm-up](technical/specs/warmup-email-campaign.md) · [plan marketing](marketing/email-marketing-plan-2026-09.md)
>
> **🟣 Update 2026-09-10 (partea a doua) — încasările prin IBAN ajung în decontări.** Importul de extras din `/admin/decontari/banca` potrivea DOAR creditele Stripe cu payout-urile; o plată venită direct de la client cădea pe categoria „altele", deci din decontări nu aveai cum să afli că o comandă fusese plătită. Acum: categorie **„Încasare client"** (orice credit care poartă IBAN-ul plătitorului; Stripe și aportul propriu rămân pe categoriile lor), potrivire cu comanda **după numărul din „detalii plată"** sau, în lipsa lui, **după suma exactă dar numai când e unică** în ultimele 90 de zile — două cazieri de 89 lei în aceeași săptămână lasă linia pentru om, nu se ghicește. Linia potrivită are link **„Confirmă plata"** care deschide comanda cu **referința tranzacției precompletată**. Importul NU marchează nimic plătit singur. Colateral, două lucruri descoperite pe cazul DMUZA: confirmarea plății trăgea comanda înapoi pe „Plătită" chiar dacă echipa începuse lucrul (acum își păstrează statusul), și nu exista niciun semnal pentru o comandă avansată în flux fără plată confirmată — acum are **banner roșu**, fiindcă în starea aia nu se emite factura și clientul nu primește nimic. Migrarea **155**. Colateral 2: `payment_method` primește „bank_transfer" la checkout dar e SUPRASCRIS cu „transfer" la confirmare, iar lista testa doar prima valoare — un transfer confirmat apărea drept **„Card"** (la fel și cash-ul); sursă unică nouă în `src/lib/admin/payment-method.ts`. ✅ **`E-260905-DMUZA` e rezolvată**: echipa a confirmat plata la 11:39, factura **EGH-0647** e emisă, emailul de confirmare a plecat. Plata venise ca încasare SEPA din Germania, 324,66 EUR la curs 5,2508 pentru o comandă de 1.646 lei — deci potrivirea pe SUMĂ n-ar fi funcționat niciodată, doar numărul comenzii din „detalii plată" traversează conversia valutară neschimbat. [changelog](changelog/2026-09-10-incasari-iban-in-decontari.md)
>
> **🔴 Update 2026-09-10 — plata prin transfer bancar nu a funcționat NICIODATĂ; reparată azi.** Declanșator: `E-260905-DMUZA` (1.646,00 RON) — client plătit prin IBAN, bani încasați, comandă marcată **„Abandonată"**, zero emailuri, plus cupon de recuperare coș trimis la 15 minute după. Trei defecte suprapuse: (1) ruta `POST /api/orders/[id]/bank-transfer` rula pe cheia anonimă, iar politicile RLS lasă un vizitator să citească/scrie doar comenzi `status='draft'` — o comandă de checkout e `pending`, deci ruta răspundea „Order not found" **inclusiv când clientul încărca dovada**; dovada în date: `payment_method` era NULL pe TOATE comenzile din tabelă; (2) butonul de confirmare era blocat până la încărcarea unui ordin de plată, deci cine pleacă să plătească din aplicația băncii nu înregistra nimic; (3) cronul auto-abandon, gândit pentru card (30 min), îngropa comanda mult înainte ca transferul să ajungă (1-3 zile lucrătoare). Colateral: `payment_proof_submitted` lipsea din lista albă `order_history.event_type`, deci insertul pica tăcut. **Acum**: status nou `awaiting_payment` (migrarea **154**), dovadă opțională, email către client cu IBAN + suma + numărul comenzii ca detalii de plată, heads-up către echipă, tab de admin **„Așteptare plată"** cu badge, și panou **„Confirmă plata"** pe orice comandă cu transfer bancar (până acum marcarea manuală exista DOAR pe comenzile telefonice). Confirmarea rulează același lanț ca plata cu cardul: factură Oblio „Transfer bancar", contact, email de confirmare, joburi, Barou. ✅ **`E-260905-DMUZA` rezolvată în aceeași zi**: echipa a confirmat plata la 11:39 cu referința `C31ZEXA26251016D`, factura **EGH-0647** emisă, emailul de confirmare plecat — primul caz real trecut prin butonul nou. **Ce a primit clientul**: verificat pe stampilele de trimitere — confirmare comandă NU, factură NU (nici nu există), doar emailul de recuperare coș cu cupon −10% la 05.09 14:15, adică singurul mesaj primit după ce plătise 1.646 lei îi sugera că a renunțat. **Cât s-a mai pierdut**: extrasele BT iunie–august au 68 de încasări, doar 5 non-Stripe, iar dintre alea o singură plată de client (726 lei, 22.08, IULIANA FUNERAR SRL, cu factura EGH 0278 deja emisă) — deci în iunie–august nu s-a pierdut nimic, cele 67 de abandonuri din august sunt reale. Extrasul pe septembrie nu era în folder, de repetat verificarea când apare. [changelog](changelog/2026-09-10-plata-transfer-bancar-reparata.md) · [instrucțiuni echipă](admin/plata-transfer-bancar.md)
>
> **🔴 Update 2026-09-09 — conținutul paginilor era invizibil pentru crawlerii AI fără JavaScript (GPTBot/ClaudeBot/PerplexityBot).** Cercetare declanșată de un articol despre „accessibility tree" ca interfață pentru agenți AI. Audit (axe-core + ariaSnapshot, 13 pagini): arborele e curat CÂND rulează JS (0 butoane/linkuri goale, 0 inputuri nelabelate) — dar `src/app/loading.tsx` la nivel de root punea ORICE pagină într-un Suspense boundary Next.js, care trimitea în HTML brut doar spinner-ul, iar conținutul real stătea ascuns într-un `<div hidden>` până rula scriptul de hidratare. Verificat separat: GPTBot, ClaudeBot, PerplexityBot — deși invitați explicit în `robots.txt` pe tot site-ul — **nu execută JavaScript** în 2026, deci vedeau doar navigarea (1.060 caractere identice, pe orice pagină). **Fix live** (PR #5, commit `64a9257`): `loading.tsx` scos din root, păstrat doar pe `/admin/` și `/comanda/` (deja `Disallow`, fără cost de crawling); `<Suspense>` inutil scos de pe homepage (componentă 100% sincronă); skip-link „Sari la conținut" reparat (era ancoră moartă pe `/`, `/servicii/`, `/comanda/status/`, care nici nu avea landmark `<main>`). **Verificat pe producție după deploy**: text vizibil fără JS a urcat de la 1.060 caractere fix la 18.839 (homepage), 8.042 (`/servicii/`), 33.224 (pagină de serviciu), 8.873 (blog), 9.468 (calculator), 4.139 (contact). ⚠️ Nu mișcă rankingul Google (Googlebot randează JS oricum, accesibilitatea nu e factor direct de ranking) — repară condiția minimă pentru citări în ChatGPT/Perplexity/Claude search (GEO), nu garantează citarea. [detalii + audit complet](seo/2026-09-09-accessibility-tree-ai-agents.md)
>
> **🟣 Update 2026-09-09 — parolă internă pe Decont/Serviciile mele în portalul topografului.** Mircea aduce un angajat care lucrează comenzile **pe contul lui** (cont separat = amânat, „durează prea mult"). „Decont lunar" și „Serviciile mele" (prețuri, onorarii) cer acum o parolă pe care o știe doar Mircea; gardul e **pe server** (cookie semnat 8h, `earnings`/`services` dau 403 `PRIVATE_LOCKED` fără el), deci URL-ul API-ului deschis direct nu-l ocolește. Comenzile nu se ating (nu expun prețul clientului). Parola e **setată din 09.09, 16:58**, din `/admin/colaboratori`. ⚠️ Deblocarea e per **browser**: pe un calculator comun Mircea apasă „Blochează" când termină. Preview-ul de admin nu arată gardul. [changelog](changelog/2026-09-09-parola-pagini-private-colaborator.md)
>
> **🔴 Update 2026-09-09 — împuternicirile serviciilor secundare + onorariul de pe cazier auto.** O comandă cu DOUĂ documente (serviciu + add-on) primea cererea de eliberare a serviciului secundar, dar **nu și împuternicirea** — codurile `addon_cazier_judiciar`, `certificat_pachet` și `extras_multilingv` lipseau din `computeDelegationItems`, iar echipa le făcea de mână (E-260907-AMV62, E-260907-EJZM7). Codurile compuse (`certificat_pachet`/`extras_multilingv`) se rezolvă acum la **slug-ul serviciului secundar**, iar textul împuternicirii urmează delegația, nu serviciul principal. În admin, add-on-urile de serviciu se recunosc după COD (apar mereu ca **Serviciu secundar**, chiar fără sub-opțiuni) și opțiunile per-document de la nivelul de sus (apostilă/traducere) arată **„Pentru: \<serviciul principal\>"** — E-260908-JQW3M. **Cazier auto**: `lawyer_fee_ron` era 0, deci facturile ieșeau fără linia „Onorariu Avocat", deși avocata e plătită 15 RON/comandă în decont — migrarea **153** (totalul comenzii nu se schimbă; cele 14 facturi emise rămân cum sunt). ⚠️ Comenzile vechi NU se re-alocă singure: rândul de împuternicire lipsă apare acum în admin — **nu-l apăsați** pe cele făcute deja manual, ar arde un număr de Barou în plus. [changelog](changelog/2026-09-09-imputerniciri-servicii-secundare-onorariu-cazier-auto.md)
>
> **🟣 Update 2026-08-26 — portal topograf: căutare după nr. depunere OCPI, cost automat la CF direct, status setat de Mircea.** Din discuția Raul–Mircea: (1) numărul de depunere raportat la „Am depus cererea la OCPI" se salvează pe comandă și e **căutabil în search-ul portalului** (peste 2 zile ridică documentul cu numărul de la ghișeu în mână); (2) la CF-urile obținute **direct online** nu mai completează nr. de depunere — încarcă PDF-ul, comanda se livrează, iar **taxa ANCPI (taxă × imobile) se înregistrează singură** dacă nu există deja; (3) card nou „**Schimbă statusul comenzii**" — subset din statusurile de admin (în lucru / depusă / problemă-info client cu notă obligatorie / eliberat / finalizată), echipa vede în admin exact ce a selectat, semnat `colaborator: Mircea`. Preview-ul de admin (`?as=`) e acum **read-only și pe server** (gaura era și la depunere). ⚠️ Clarificare onorariu: **extras CF are corect 0** — „20 lei" = costul de eliberare ANCPI, nu tarif de colaborator (migrarea 148 greșită, anulată de 149 în aceeași seară). **Decontul refăcut pe modelul real 50/50** cu sursă unică `src/lib/collaborator/settlement.ts` (TVA 21% → −taxe OCPI → −impozite 16%+16% → split): `/colaborator/decont` și `/admin/colaboratori` arată aceleași cifre, verificate contra decontului de referință (4.316,61/parte) și pe augustul live (105 comenzi / 11.280,07 → 2.911,44/parte); următorul decont se citește din pagină, nu se mai calculează ad-hoc. [changelog](changelog/2026-08-26-portal-topograf-ocpi-search-status.md) · [spec](technical/specs/cereri-ocpi-colaborator.md) · [decont](operations/decont-mircea-2026-08-26.md)
>
> **🔴 Update 2026-08-19 — Google Ads: 3 din 4 campanii respinse pe politica documentelor guvernamentale.** Repornit contul pe 18.08 cu 3 campanii (constatator, constatator cu istoric, cazier fiscal) + una de cadastru/topograf pe 19.08. A doua zi Google a respins **toate cele trei campanii pe documente**; a trecut **doar cadastrul/PAD**, fiindcă anunțul vinde munca topografului, nu obținerea documentului de la instituție. Certificarea cerută e doar pentru instituții sau furnizori delegați — cale închisă pentru noi; cont nou = eludare, deci suspendare. Cele 3 sunt pe pauză (buget cont 320 → 40 lei/zi), cheltuit total 79,70 lei / 0 conversii. ⚠️ Capcană nouă: **sitelinkurile la nivel de cont** se lipesc pe toate campaniile — 3 respinse pe constatator au apărut ca „Elemente (3)” pe campania de cazier fiscal; toate întrerupte, înlocuite cu sitelinkuri **per campanie**. Certificatul de urbanism (recomandat inițial) **iese din plan** — e emis de primărie, aceeași politică. [analiză + stare](ads/2026-08-18-campanii-live-status.md) · [cercetare topograf](ads/2026-08-18-topograf-cadastru-research.md)
>
...
>
> **🟣 ANCPI, 21 august — extrasele CF le depune TOPOGRAFUL, nu worker-ul.** Portalul ePay e picat din 13 iulie, iar comenzile se adunaseră: **109 plătite și nelivrate**, cea mai veche din 14 iulie. Platforma generează acum **cererea completată (Anexa 6)** din datele comenzii, denumită în convenția lui (`cf 101010 - Baile Govora-Valcea.pdf`, el citește CF/UAT/județ DIN DENUMIRE), câte una per imobil, cu **ZIP pe județ**, raportarea depunerii (nr. înregistrare + taxă precompletată: 20 lei extras CF, 15 lei plan cadastral) și livrare automată la încărcarea PDF-ului. Lista lui e **cea mai veche prima**, cu marcaj **URGENT** (migrarea 145) pentru clienții care scriu supărați. **Prima noapte: 39 de comenzi finalizate, 33 de e-mailuri livrate, 0 documente lipsă.** Cele 109 joburi ANCPI sunt pe `NEEDS_OPERATOR` (NU `PENDING` — retry = dublă plată) și coada e oprită pentru comenzi noi (`HANDED_OVER_TO_COLLABORATOR`). ⚠️ **Când revine portalul: flagul înapoi pe `false` + de decis ce se face cu lucrările aflate la topograf.** [changelog](changelog/2026-08-21-cereri-ocpi-topograf.md) · [spec](technical/specs/cereri-ocpi-colaborator.md)
>
...
> - **Procese noi de lucru**: vezi „Cum funcționăm de azi" în changelog — Solicită documente cu auto-standby, auto-procesare la generarea cererii, dropdown status, deploy pe loturi (docs/preview fără build).
> - **🔒 Cazier PJ cere buletin administrator + selfie** (migrația 100): `personalKyc` fusese dezactivat direct în DB (fără migrare, înainte de 28.05) → E-260708-VC4GH a comandat FĂRĂ act/selfie, iar guard-ul din `/submit` (gate pe `personalKyc.enabled`) sărea peste PJ. Re-activat cu fluxul simplificat (ca la cazier-auto). Clientul BLU IT contactat prin noul sistem de mai jos.
> - **🟣 „Solicită documente"** (migrațiile 101+102, spec `technical/specs/document-request-system.md`): admin bifează documentele cerute (pre-bifate cele necesare serviciului), clientul primește email + banner pe status cu link single-use (7 zile), pagina `/reincarca-poza` acceptă multi-doc (poze + PDF acte firmă). Comanda intră automat în `standby` („Așteptare client", vizibilă în tab-ul În procesare) și **revine singură la statusul anterior** când clientul încarcă tot + **email pe contact@**. Cerere vizibilă persistent pe comandă în admin (progres ✓, Copiază/WhatsApp). Testat live pe E-260708-VC4GH.
> - **🚨 Env fix:** `NEXT_PUBLIC_APP_URL`/`SITE_URL` pe Vercel erau `eghiseul-ro.vercel.app` (SSO!) → linkurile din emailuri duceau la login wall. Corectate la `https://eghiseul.ro` + redeploy. Toate mediile verificate curate.
> - **📧 Design branded pe emailuri tranzacționale** (`branded-layout.ts`, ca emailul de confirmare): document gata (ONRC/ANCPI/colaborator — înainte HTML gol), solicitare documente, notificare internă. CTA „document gata" duce la status page pre-completat (fără cont), nu la /account.
>
...
> - **✅ Paritate admin listă comenzi cu cazierjudiciaronline** (fetele obișnuite cu acel layout): chips „Filtre rapide" (Expirate/Deadline<48h/Cu cupon) + chips „Stadiu" (Documente generate/Depus/Primit/Gata livrare, adaptate la pipeline eghiseul) + coloană **Termen** (deadline color-coded) + badge **„Fără factură"** + icon cupon + **note echipă vizibile** (preview trunchiat sub client + tooltip) + coloană Acțiuni cu **dialog „Detalii"** (preview comandă fără navigare) + search după nume/telefon. Helper `order-quick-filters.ts` + 8 teste. Doc: [`admin/orders-list-parity.md`](admin/orders-list-parity.md).
...
> - **🔴🎉 FACTURARE REPARATĂ — `EGH-0001` emisă:** cauza reală a facturilor lipsă = **kill-switch** `admin_settings.invoicing.oblio_enabled=false` (rămas de la testare, NU webhook-ul/seria). Pornit cu acordul userului → backfill mariacercel = prima factură reală pe seria EGH. Toate comenzile plătite primesc factură automat de acum.
> - **✅ Bug-uri admin găsite + reparate prin testare reală în browser** (logat ca super_admin, Playwright): (1) notă echipă crash — `order_history.changed_by` uuid→text (migrația 094); (2) istoric+note nu se afișau NICIODATĂ — GET selecta coloane inexistente `from_status/to_status` (schema are `old_value/new_value`); (3) status route nu loga schimbările (scria în aceleași coloane inexistente); (4) buton „Marchează documente generate" mort (400 — alias acțiune lipsă); (5) parolă admin resetată (fișier în Downloads).
> - **✅ Status-uri granulare add-on** (migrația 093): `la_tradus`, `la_legalizat`, `la_apostila_notari`, `eliberat_apostila_haga` — în dropdown + badge-uri listă/detaliu. Testat: schimbare status → `la_tradus` + istoric logat corect.
> - **✅ Paritate detaliu comandă:** Note Echipă mutate SUS (ca sora) + link-uri Stripe în Plata (Checkout Session + Payment, live/test auto) + „Platita la"/„Data comanda". Dialog Detalii pe listă cu link factură.
> - **✅ Registru numere verificat pe comandă reală:** contract `004324` + delegație `SM5827` alocate AUTOMAT la generare (mariacercel); delegații distincte per serviciu (`p_service_type` — cazier vs apostilă = numere diferite); rezervare manuală la `/admin/registru`. Cross-platform (ecazier/cazierjudiciar din același registru) = neconstruit, proiect separat.
> - **🔴 Rămâne:** reordonare completă carduri detaliu 1:1 ca sora (Note sus ✓, restul pe faze); rotire `sk_live`; storno facturi test Oblio; batch import restul comenzilor plătite WP; registru cross-platform; email confirmare comandă (nu se trimite); perf listă admin.
>
...
> - **Wizard feedback + OCR + ANAF (2026-06-25):** la `Continuă` pe pas invalid → listă „ce lipsește" + scroll (PersonalData/KYC) + toast universal. **OCR din PDF** reparat (regex strip data-URL generic — PDF-urile act se extrag acum). **RO CEI** clarificat („documentul care confirmă adresa"). **Fix ANAF firmă:** `isCompanyActive()` folosește registrul de inactivi (`statusInactivi`) + exclude radiat/dizolvat/lichidare, NU cuvântul „INREGISTRAT" → firme cu sediu transferat („TRANSFER(SOSIRE)") nu mai sunt blocate fals (SOLTERA PLAST, CUI 49754074). Notă: `infocui.ts` = nume istoric, folosește ANAF (nu infocui.ro). Teste: `tests/unit/company/is-active.test.ts`.
> - **KYC/stare civilă (2026-06-25):** scos „Țara folosit actul" la civil (migrație 081); label „Ce tip de act dețineți"; naștere/căsătorie „vechiul certificat" = doar Pierdut (celibat 3); **scan mode cere setul COMPLET per tip** (CI vechi=față; CI nou=față+spate+RO CEI PDF; pașaport=pagina) → repară bug-ul „CI nou cu doar fața / aceeași poză 2×"; RO CEI PDF obligatoriu la CI nou civil. **Ruta manuală** cere acum actul **față + spate** la pasul KYC (act_identitate + act_identitate_back). TODO rămas: „Vezi specimen" pe tipul de act (nevoie 3 imagini CI vechi/nou/pașaport în public/images/specimens/).
