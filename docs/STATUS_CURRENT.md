...
> - **Admin comandă — val de polish pe comenzi reale** (changelog §22–25): ciclul „Solicită documente" confirmat cap-coadă (upload client → auto-exit standby → email echipă); fix badge PF/PJ + CUI; generarea cererii mută automat comanda pe „În procesare" (§23); dropdown rapid de status în header + cardul vechi eliminat, Detalii Serviciu lângă Note Echipă, Date imobil + Facturare pe dreapta la CF; constatator pe firmă arată firma-țintă; rândurile Urgenta/Livrare/Termen ascunse la serviciile instant; fix dropdown servicii gol pe lista de comenzi.
