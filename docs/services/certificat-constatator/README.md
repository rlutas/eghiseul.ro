...
- 🔎 **Cum deosebești varianta „fonduri IMM" de „de bază" în PDF-ul livrat** (verificat 2026-08-12 pe 4 documente reale): prima pagină se numește „CERTIFICAT CONSTATATOR" la AMBELE, deci titlul nu spune nimic; datele financiare (cifră de afaceri, salariați, profit) apar tot în ambele. Diferența e structura — **IMM ≈ 13 pagini + secțiunea „ISTORIC PE SEDII SI/SAU ACTIVITATI AUTORIZATE"**, de bază ≈ 5 pagini fără istoric. Scopul cerut de client se tipărește la final („fiindu-i necesar la …"), deci două certificate IMM pe aceeași firmă diferă doar prin scop, dată și codul de verificare — arată aproape identice, e normal (caz E-260810-ZHVRX vs E-260811-GC4MA, ELIEZER PROD SRL).
- ⏱️ **IMM se eliberează acum INSTANT** (ambele comenzi de mai sus, ~4 minute de la plată), deși până în iunie 2026 trecea prin backoffice-ul ONRC cu termen de ~24h lucrătoare. Verifică realitatea înainte să promiți 24h în wizard.
- ⚠️ **Capcană în worker** (`worker-onrc`, `src/onrc/api-submit.ts` → `buildStep4Doc`): dacă subtipul cerut nu se găsește în nomenclatorul ONRC, codul cade pe „de bază" — deci o redenumire la ONRC ar face ca cererile IMM să primească tăcut varianta de bază. Azi mapează corect.
