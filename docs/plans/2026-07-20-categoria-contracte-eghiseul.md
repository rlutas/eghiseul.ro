...
## ⚠️ Limita serviciului — ce NU facem (corecție Raul, 20.07)

**Nu obținem certificatele de atestare fiscală în locul clienților.** Ar însemna împuternicire de la fiecare parte, drum la fiecare primărie, fulfillment manual — exact opusul produsului automat.

**Ce facem în schimb — și e mai valoros decât pare:**
1. **Pregătim cererea completată** (ITL-010 / ITL-011) pentru fiecare parte, gata de semnat și depus.
2. **Spunem exact UNDE se depune**, per parte, în funcție de domiciliul fiecăruia.

Punctul 2 e diferențiatorul real, pentru că regula nu e evidentă: **se merge la primăria de DOMICILIU, nu la reședința de județ.** Exemplu concret (Raul): domiciliu în comuna **Odoreu, județul Satu Mare** → se merge la **Primăria Odoreu**, nu la Satu Mare oraș. Vânzătorul și cumpărătorul pot fi din UAT-uri complet diferite, deci fiecare are propria destinație.

Asta transformă checklistul din „listă generică de pași" în **instrucțiuni personalizate: cine, cu ce hârtie, la ce adresă**. Niciun competitor nu face asta — toți dau ghiduri generice de tip „mergi la primărie".

Depinde de o bază de date cu UAT-urile și datele de contact ale primăriilor — vezi §6.4.
...
### 6.4 Baza „unde depui" (UAT + primării)

Alimentează instrucțiunile personalizate din §1. Ce avem deja:

- **`src/lib/data/romania-localities.json`** — 42 județe, **13.251 localități** (doar nume). Odoreu e prezent ✅
- `src/lib/data/locality-fuzzy-match.ts` — potrivire aproximativă pe nume (deja folosit la curieri)
- `src/lib/data/romania-counties.ts`, `src/lib/ancpi/judete.ts`

**Ce lipsește — piesa esențială:** maparea **localitate → UAT cu primărie**. Sunt 13.251 de localități, dar doar ~3.200 de UAT-uri au primărie: un sat aparține de comuna X, iar acolo se depune. Fără mapare, nu putem spune corect „mergi la Primăria Odoreu" pentru cineva din satul vecin arondat aceleiași comune.

Peste asta trebuie: adresă, program, telefon, plus dacă UAT-ul are **DITL separat** (orașele mari: DITL Sector 3, SPIT Constanța) sau se merge direct la primărie.

**Sursele candidate** (research în curs): SIRUTA/INS, data.gov.ro, geo-spatial.org, OpenStreetMap, Google Places. Contează licența — trebuie utilizabile comercial.

**Strategie de acoperire progresivă**, ca să nu blocăm lansarea: pornim cu județele/orașele mari (acoperă majoritatea tranzacțiilor), iar pentru restul afișăm instrucțiunea generică („Primăria comunei de domiciliu — Direcția de Impozite și Taxe Locale") fără adresă. Completăm pe măsură ce apar comenzi reale.

### 6.5 Ce e efectiv de scris
