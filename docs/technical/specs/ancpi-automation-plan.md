...
> ⚠️ **Incident 2026-07-13 (E-260710-F3AYS):** CF vechi → extras livrat pe unitatea greșită. Plan complet de dezambiguizare (worker + wizard + admin): `worker-ancpi/docs/cf-vechi-dezambiguizare.md`. Regula „exact 1 rezultat" din §5.3/linia 231 NU e încă implementată în worker.
...
**Job manual din admin (2026-07-14):** după ce operatorul găsește CF-ul, NU mai emite extrasul de mână — pe pagina comenzii (`/admin/orders/[id]`, sub „Date imobil", pentru `identificare-imobil` / `identificare-imobile-proprietar` / `extras-carte-funciara` plătite) există **„Generează extras CF (worker ANCPI)"**: formular județ/UAT/identificator (CF/CAD/TOPO, cu avertizări `checkCf` pt. colectivă/format vechi) → `POST /api/admin/orders/[id]/ancpi-create-job` inserează `ancpi_jobs` PENDING (prod 14200, `detail` identic cu `ensure-ancpi-job.ts`, eveniment `created_manual`) → workerul îl preia la următorul poll și livrează automat. Guard-uri: doar comenzi plătite (consumă 1 punct), 409 dacă există deja job (index unic `order_id`), `requirePermission('orders.manage')`. UI: `src/app/admin/ancpi/AncpiCreateJob.tsx`.
