...
- **Probă de portal (2026-07-28):** worker-ul raportează starea SSO-ului ONRC la `/api/onrc/pending?portal=up|down` → ferestre în `platform_outages` (badge public + „⏸ ONRC din …" în admin). Probă la 15 min (`PROBE_INTERVAL_MS`), cache între tick-uri, `down` abia după 2 eșecuri consecutive; contactul real cu portalul în timpul unui job ține loc de probă. Până acum tabela avea doar rânduri `ancpi` — căderile ONRC erau invizibile.
- **Parola contului ONRC (incident 14.09.2026):** portalul a cerut resetarea parolei → Keycloak `invalid_grant` → worker-ul cădea pe fallback-ul de browser, care **nu merge în producție** (lockfile `playwright@1.60.0` vs imagine `v1.48.0-jammy` — reparat la 1.60.0 — și fără `storageState.json` pe Railway). Acum jobul ajunge `NEEDS_OPERATOR` cu mesajul real. **Procedură:** resetează parola pe portal → `railway variables --set 'ONRC_PASSWORD=…'` din `~/Projects/worker-onrc` (redeploy automat) → `/admin/onrc` → „↻ Reîncearcă automat". Tag-ul imaginii Docker urmează **`package-lock.json`**, nu `^` din `package.json`. ⚠️ `railway variables --json` dă valoarea BRUTĂ — `ONRC_USERNAME` avea un spațiu la final (worker-ul face `trim()`, testele manuale nu). Vezi [changelog](../../changelog/2026-09-14-onrc-worker-parola-imagine-playwright.md).
- **Cadența retrieve (14.09.2026):** după depunere, `/api/onrc/pending` dă jobul la verificare **la 1 min în primele 15 min** (`awaiting_since`), apoi la 3 min. Id-ul cererii (`number`) apare la ONRC la câteva secunde–minute după plată; worker-ul așteaptă până la 40 s după el înainte să parcheze jobul, altfel admin-ul arăta „undefined".
