...
| [Clientul nu își poate face cont](./cont-client-blocat.md) | Ce spui clientului blocat la înregistrare (linkul de urmărire fără cont), cum îi creezi contul din Supabase cu „Auto Confirm User", cum confirmi un cont vechi neconfirmat, și ce NU îi spui | Procedură (2026-09-17) — blocajul de fond rămâne până la SMTP custom |
...
| AWB label download (PDF) | `/api/admin/orders/[id]/awb-label` | Print-ready labels; eticheta se descarcă server-side (Fan acceptă token DOAR pe header `Authorization: Bearer` — `?token=` dă 401) |
| AWB cancellation | `/api/admin/orders/[id]/cancel-awb` | Cancel AWB and revert status |
| AWB manual + edit/replace | `/api/admin/orders/[id]/set-awb` | Manual AWB (DHL/Poșta/internațional) + „Modifică / înlocuiește AWB-ul" în cardul verde după salvare (paritate CJO, 2026-08-03); înlocuirea se loghează în `order_history`. **Trece comanda pe `shipped` + `shipped_at`** ca generarea automată Fan/Sameday — `shouldMarkShippedOnAwb()` din `src/lib/orders/shipping-status.ts`, listă neagră: `draft`/`abandoned`/`cancelled`/`refunded` + `shipped`/`completed` (2026-08-06) |
...
| PDF document preview (2026-06-12) | `/api/admin/orders/[id]/preview-document` + `src/lib/documents/docx-to-pdf.ts` | Preview converts DOCX→PDF once and caches it in S3 (`<key>.preview.pdf`), served inline — 1:1 layout, native browser print. Engine: CloudConvert (`CLOUDCONVERT_API_KEY`, optional) or local LibreOffice (dev); falls back to mammoth HTML preview (`?format=html`) when unavailable. Cache invalidated on document regeneration |
| Native PDF fill for ANAF cerere (2026-06-12) | `src/lib/documents/cazier-fiscal-cerere-pdf.ts` | The cazier-fiscal cerere PDF is filled natively with pdf-lib (no converter/account needed, runs on Vercel): frozen base PDF + per-line segment map, lines redrawn with natural flow (labels regular, values bold, Liberation Sans for diacritics). Generated at document-generation time and uploaded as the preview PDF. Regenerate assets after template changes: `scripts/build-fiscal-cerere-pdf-template.ts` |
...
| `/api/admin/orders/[id]/set-awb` | POST | `orders.manage` | Manual AWB save/replace (logged in order_history) + trece comanda pe `shipped` |
...
| `/api/admin/orders/[id]/messages` | GET/POST | `orders.view` / `orders.manage` | Firul de mesaje cu clientul (vizibil clientului, email la fiecare mesaj). Ghid: `mesaje-client.md` |
| `/api/collaborator/orders/[id]/messages` | GET/POST | rol `collaborator` pe comandă | Topograful scrie direct clientului; preview admin read-only |
