...
| Users | `/admin/users` | User management (planned) |
| Settings | `/admin/settings` | System settings (planned) |

### AWB Generation Flow

1. Go to `/admin/orders` and find the order
2. Click the order row to open detail page
3. Click "Genereaza AWB" button (auto-detects Fan Courier or Sameday)
4. AWB is generated and order status changes to "Expediat"
5. Use "Printeaza Eticheta" to download the label PDF
...
+-----------------------------------------------------------+
|  +------------+ +------------+ +------------+ +------------+|
|  | Comenzi    | | Venituri   | | Clienti    | | Actiuni    ||
|  | Astazi     | | Luna Asta  | | Noi        | | Pending    ||
|  | 127        | | 45,890 RON | | 34         | | 8          ||
|  | +12%       | | +8%        | | +15%       | | -2         ||
|  +------------+ +------------+ +------------+ +------------+|
|                                                             |
|  +-----------------------------+------------------------+  |
|  | Comenzi pe Status           | Activitate Recenta     |  |
|  |                             |                        |  |
|  | ||||| Finalizate (78%)      | - Comanda #E-...       |  |
|  | ||| In procesare (12%)      |   plata confirmata     |  |
|  | || In livrare (7%)          | - Comanda #E-...       |  |
...
|  | Comenzi Recente                                        | |
|  +------+----------+-------------+----------+-----------+ |
|  | ID   | Client   | Serviciu    | Status   | Actiuni   | |
...
**Data Sources:**
- **Comenzi Astazi:** Count of orders with `created_at` = today
- **Venituri Luna Asta:** Sum of `amount_paid` for current month
- **Clienti Noi:** Count of profiles created this month
- **Actiuni Pending:** Count of orders needing attention (bank transfers, failed delivery)
...
- In procesare (processing, document_ready)
- In livrare (shipped)
...
- Service
- Status Badge
- Actions (Vezi detalii, Genereaza AWB if applicable)
...
+-----------------------------------------------------------+
|  +-------------------------------------------------------+ |
|  | Filtre:  [Status v] [Data v] [Serviciu v] [Curier v]  | |
|  | Cautare: [___________________________________] Search  | |
|  +-------------------------------------------------------+ |
|                                                             |
|  +-------------------------------------------------------+ |
|  | Actiuni: [Export CSV] [Export Excel]                    | |
|  +-------------------------------------------------------+ |
|                                                             |
|  +------+---------+----------+---------+----------+------+ |
|  | ID   | Client  | Serviciu | Plata   | Status   | Act. | |
|  +------+---------+----------+---------+----------+------+ |
|  | E-.. | Ion P.  | Cazier   | Card OK | Shipped  | Vezi | |
|  | E-.. | Maria S.| Extras CF| Pending | Pending  | Vezi | |
|  | E-.. | Dan A.  | Cert.Con | Transfer| Pending  | Vezi | |
|  +------+---------+----------+---------+----------+------+ |
|                                                             |
|  Pagina 1 din 23   [Prev] [1] [2] [3] ... [23] [Next]     |
+-----------------------------------------------------------+
```
...
| Plata | Payment method + status | Yes |
| Status | Order status badge | Yes |
| Data | Created date | Yes (default desc) |
| Actiuni | View, Edit, Delete (soft) | No |
...
|  | Informatii Comanda                                     | |
|  +-------------------------------------------------------+ |
|  | Status: [Pending v]           Data: 13 Feb 2026        | |
|  | Serviciu: Cazier Fiscal       Pret: 180 RON            | |
|  | Optiuni: Procesare Urgenta (+99 RON)                   | |
...
|  | Adresa: Str. Exemplu 12, Bl. A, Sc. B, Ap. 5          | |
|  |          Bucuresti, Sector 1, 012345                   | |
|  +-------------------------------------------------------+ |
|                                                             |
|  +-------------------------------------------------------+ |
|  | Plata                                                  | |
|  +-------------------------------------------------------+ |
|  | Metoda: Card (Stripe)            Status: Platit        | |
|  | Suma: 279 RON                    Data: 13 Feb 14:30    | |
...
|  | Tracking: [Link]                                       | |
|  | [Printeaza Eticheta] [Tracking] [Anuleaza AWB]        | |
|  +-------------------------------------------------------+ |
|                                                             |
|  +-------------------------------------------------------+ |
|  | Documente KYC                                          | |
|  +-------------------------------------------------------+ |
|  | CI Fata: [Vizualizare] [Descarca]                     | |
|  | CI Spate: [Vizualizare] [Descarca]                    | |
|  | Selfie: [Vizualizare] [Descarca]                      | |
...
|  | - 13 Feb 14:30 - Plata confirmata (Card, 279 RON)     | |
|  | - 13 Feb 14:25 - Comanda creata                       | |
|  +-------------------------------------------------------+ |
|                                                             |
|  +-------------------------------------------------------+ |
|  | Notite Interne (Admin only)                            | |
|  +-------------------------------------------------------+ |
|  | [Adauga notita...]                                    | |
|  |                                                       | |
|  | - 14 Feb 09:00 (Admin): Client a sunat, document     | |
|  |   lipseste semnatura. Refacut.                        | |
|  +-------------------------------------------------------+ |
+-----------------------------------------------------------+
```
...
|                                                        |
| [Printeaza Eticheta] [Track Shipment] [Anuleaza AWB]  |
+-------------------------------------------------------+
```

**Actions:**
- **Printeaza Eticheta:** Download PDF label (`GET /api/admin/orders/[id]/awb-label?format=pdf`)
- **Track Shipment:** Open tracking page in new tab
- **Anuleaza AWB:** Cancel AWB and reset order to `document_ready` (`POST /api/admin/orders/[id]/cancel-awb`)
...
|                                                        |
| Adresa incompleta: lipseste judetul                    |
|                                                        |
| [Incearca din nou]                                     |
+-------------------------------------------------------+
```
...
**Component:** `src/components/admin/awb-section.tsx`
...
export function AWBSection({ order, onSuccess }: AWBSectionProps) {
  ...
  const handleGenerateAwb = async () => {
    if (!order.courier_provider) {
      toast.error('Nicio metoda de livrare selectata');
      return;
    }
  ...
  const handlePrintLabel = async () => {
    ...
    try {
      ...
      toast.error('Nu s-a putut descarca eticheta');
    }
  };
  ...
  const handleCancelAwb = async () => {
    ...
    const confirmed = confirm('Sigur doriti sa anulati AWB-ul?');
    if (!confirmed) return;
...
+-------------------------------------------------------+
| Status: Asteapta Verificare                            |
| Suma: 279 RON                                         |
| Referinta: PAY-20260213-00123                          |
|                                                        |
| Dovada incarcata: [Vizualizare] [Descarca]             |
| Data incarcare: 13 Feb 2026, 14:35                     |
|                                                        |
| [Confirma Plata] [Respinge]                            |
+-------------------------------------------------------+
```
...
4. **Admin confirms:** Clicks "Confirma Plata"
...
**Component:** `src/components/admin/payment-verification.tsx`
...
export function PaymentVerification({ order, onVerified }: PaymentVerificationProps) {
  ...
  const handleConfirmPayment = async () => {
    ...
    try {
      ...
      if (!data.success) {
        throw new Error(data.error?.message || 'Eroare la confirmarea platii');
      }

      toast.success('Plata confirmata cu succes');
      onVerified?.();
    } catch (err) {
  ...
  const handleRejectPayment = async () => {
    if (!rejectReason.trim()) {
      toast.error('Introduceti un motiv pentru respingere');
      return;
    }
    ...
    try {
      ...
      if (!data.success) {
        throw new Error(data.error?.message || 'Eroare la respingerea platii');
      }

      toast.success('Plata respinsa');
      onVerified?.();
    } catch (err) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Plata - Transfer Bancar</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Badge variant="warning">Asteapta Verificare</Badge>
            </div>
            <div>
            ...
            <div className="col-span-2">
              <p className="text-sm text-neutral-500">Referinta</p>
            ...
            <div className="p-4 bg-neutral-50 rounded-lg border border-neutral-200">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="font-medium">Dovada incarcata</p>
                    <p className="text-sm text-neutral-500">
...
+-----------------------------------------------------------+
|  +-------------------------------------------------------+ |
|  | Filtre:  [Rol v] [Status KYC v] [Data Inreg. v]       | |
|  | Cautare: [___________________________________] Search  | |
|  +-------------------------------------------------------+ |
|                                                             |
|  +------+-------------+--------+----------+--------+------+|
|  | ID   | Nume        | Email  | KYC      | Comenzi| Act. ||
|  +------+-------------+--------+----------+--------+------+|
|  | 1234 | Ion Popescu | ion@.. | Verif.   | 5      | Vezi ||
|  | 1235 | Maria S.    | maria@ | Partial  | 2      | Vezi ||
|  | 1236 | Dan Andrei  | dan@.. | Neverif. | 0      | Vezi ||
|  +------+-------------+--------+----------+--------+------+|
|                                                             |
|  Pagina 1 din 45   [Prev] [1] [2] [3] ... [45] [Next]     |
+-----------------------------------------------------------+
```
...
**Access:** Admin role only (`settings.manage` permission required)
...
+-----------------------------------------------------------+
|  +------------------+------+------------+---------+-------+ |
|  | Serviciu         | Pret | Optiuni    | Activ   | Act.  | |
|  +------------------+------+------------+---------+-------+ |
|  | Cazier Fiscal    | 180  | 3 optiuni  | Da      | Edit  | |
|  | Extras CF        | 290  | 2 optiuni  | Da      | Edit  | |
|  | Cert. Const.     | 150  | 1 optiune  | Da      | Edit  | |
|  +------------------+------+------------+---------+-------+ |
|                                                             |
|  [Adauga Serviciu Nou]                                      |
+-----------------------------------------------------------+
...
#### 4. Date firma & Avocat (Implemented)

**Manage company data, lawyer data, signatures, and document counters:**
...
|                                                             |
|  ┌─ Date firma ─────────────────────────────────────────┐  |
|  | Nume firma:     [EDIGITALIZARE SRL]                  |  |
|  | CUI:            [RO49278701]                         |  |
|  | Nr. Inregistrare:[J30/1097/2023]                     |  |
|  | Adresa:         [Jud. Satu Mare, ...]                |  |
|  | IBAN:           [RO82BTRL...]                        |  |
...
|  | Semnatura prestator:                                  |  |
|  | ┌───────────────────┐                                |  |
|  | │ [Signature Image] │  [X Sterge]                    |  |
|  | └───────────────────┘                                |  |
|  | [Incarca semnatura]  (PNG, max 5MB)                  |  |
|  |                                                       |  |
|  | [Salveaza datele firmei]                              |  |
|  └───────────────────────────────────────────────────────┘  |
|                                                             |
|  ┌─ Date avocat ────────────────────────────────────────┐  |
|  | Nume cabinet:   [Cabinet de avocat ...]              |  |
|  | Nume avocat:    [Tarta Ana-Gabriela]                 |  |
|  | Adresa prof.:   [Satu Mare, ...]                     |  |
|  | CIF:            [40198820]                           |  |
|  | Serie imput.:   [SM]                                 |  |
|  | Onorariu:       [15]                                 |  |
|  |                                                       |  |
...
|  | Stampila avocat:                                      |  |
|  | ┌───────────────────┐                                |  |
|  | │ [Stamp Image]     │  [X Sterge]                    |  |
|  | └───────────────────┘                                |  |
|  | [Incarca stampila]  (PNG, max 5MB)                   |  |
|  |                                                       |  |
...
|  |  ┌──────────────┐  ┌──────────────┐                  |  |
|  |  │    4256      │  │    5738      │                  |  |
|  |  │ Nr. contract │  │ Nr. imput.   │                  |  |
|  |  └──────────────┘  └──────────────┘                  |  |
|  |  (se incrementeaza automat la generarea documentelor) |  |
|  └───────────────────────────────────────────────────────┘  |
+-----------------------------------------------------------+
```
...
1. Admin clicks "Incarca semnatura" (or "Incarca stampila")
2. File input accepts PNG, JPG, or WebP images (max 5MB)
3. Client requests presigned URL from `POST /api/upload` with `category: 'signatures'`
4. File is uploaded directly to S3 at `signatures/{type}/{timestamp}.png`
5. S3 key is saved to `admin_settings` (`company_data.signature_s3_key`, `lawyer_data.signature_s3_key`, or `lawyer_data.stamp_s3_key`)
6. Preview is loaded via presigned download URL from `GET /api/upload/download`
7. Admin can remove a signature by clicking the X button (removes S3 key from settings)

**Admin Settings Keys (stored in `admin_settings` table):**

| Key | S3 Signature Fields | Description |
|-----|---------------------|-------------|
| `company_data` | `signature_s3_key` | Company/prestator signature PNG |
| `lawyer_data` | `signature_s3_key` | Lawyer signature PNG |
| `lawyer_data` | `stamp_s3_key` | Lawyer stamp PNG |
...
**Implementation:**
- S3 key generator: `generateSignatureKey()` in `src/lib/aws/s3.ts`
- Upload API: `POST /api/upload` with `category: 'signatures'` and `signatureType`
- Settings UI: `src/app/admin/settings/page.tsx` (DateFirmaTab component)
- Signature types: `company_signature`, `lawyer_signature`, `lawyer_stamp`
...
|  Email Templates                                            |
+-----------------------------------------------------------+
|  Template: [Confirmare Comanda v]                           |
|                                                             |
|  Subiect: [Comanda confirmata #{{order_id}}]               |
|                                                             |
|  Corp:                                                      |
|  +-------------------------------------------------------+ |
|  | Buna {{customer_name}},                                | |
|  |                                                        | |
|  | Comanda ta #{{order_id}} a fost confirmata.           | |
|  | Vei primi documentele in {{delivery_days}} zile.      | |
|  +-------------------------------------------------------+ |
|                                                             |
|  Variabile disponibile: {{order_id}}, {{customer_name}}... |
|                                                             |
|  [Previzualizare] [Salveaza] [Resetare la Default]         |
+-----------------------------------------------------------+
```
...
+-----------------------------------------------------------+
|  GDPR                                                       |
|  Draft Cleanup: [7 zile v]                                  |
|  Data Retention: [10 ani v]                                 |
|  Auto-cleanup activ: Da                                     |
|                                                             |
|  Notificari                                                 |
|  Email notificari pentru clienti: Da                        |
|  SMS notificari pentru statusuri importante: Da             |
|  Admin alerts pentru plati noi: Da                          |
|                                                             |
|  Mentinere                                                  |
|  [Ruleaza Cleanup Manual]                                   |
|  [Export Date (GDPR)]                                       |
|  [Backup Database]                                          |
|                                                             |
|  [Salveaza Setari]                                          |
...
**File:** `supabase/migrations/022_admin_rls.sql`
...
export default async function AdminDashboardPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatsCard title="Comenzi Astazi" value={todayOrders?.length || 0} />
...
- `supabase/migrations/022_admin_features.sql`
...
**Files:**
- `src/components/admin/awb-section.tsx`
- `src/app/admin/orders/[id]/page.tsx` (update)
...
- `src/app/api/admin/orders/[id]/verify-payment/route.ts` (update existing)
...
**Files:**
- `docs/technical/api/admin-api.md` (NEW)
- `docs/admin/user-guide.md` (NEW)
...
**Document Status:** Partially Implemented (Phases 1-5 Complete)
