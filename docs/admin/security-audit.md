...
if (!orderEmail) {
  return NextResponse.json(
    { error: 'Invalid order', message: 'Comanda nu contine email valid' },
    { status: 400 }
  );
}
...
## Signature Storage Security (2026-02-17)

### Architecture

Signature images are stored in AWS S3, not in the database:

| Signature Type | Storage Location | Access Pattern |
|---------------|------------------|----------------|
| Client drawn signature | `order.customer_data.signature_base64` (JSONB in orders table) | Read by admin during document generation |
| Company predefined signature | S3: `signatures/company_signature/{uuid}.png` | Downloaded server-side by generate-document API |
| Lawyer predefined signature | S3: `signatures/lawyer_signature/{uuid}.png` | Downloaded server-side by generate-document API |
| Lawyer stamp | S3: `signatures/lawyer_stamp/{uuid}.png` | Downloaded server-side by generate-document API |

### Security Considerations

- **S3 keys stored in admin_settings**, not in client-accessible data
- **Presigned URLs** used for admin preview (15-minute expiry, server-generated)
- **Client signature base64** is stored in order JSONB -- accessible only through authenticated API endpoints
- **Contract preview API** (`/api/contracts/preview`) does NOT include real company/lawyer signatures -- only placeholder boxes are shown to the customer
- **Download of predefined signatures** happens server-side only (in generate-document route), never exposed to client

### Potential Risks

| Risk | Severity | Mitigation |
|------|----------|------------|
| Client signature persisted as base64 in orders table | LOW | Data is within authenticated order context, covered by RLS |
| Company/lawyer S3 keys visible to super_admin in settings | LOW | Expected behavior; settings require `settings.manage` permission |
| Contract preview API has no auth | MEDIUM | By design for guest accessibility; only client-provided data is reflected, no secrets |
