...
| 03.09 15:15 | **PUBLICAT.** Blocajele reale, în ordine: (1) card pe contul de reclame — Raul l-a adăugat de 5 ori „OK", dar apărea **„Not verified"** în Billing → Payment methods (Visa ····0827); rezolvat cu „Verify card" (3DS/autorizare mică); (2) **„Phone number required"** — telefon verificat pe contul de reclame (Account overview → Verify phone number, SMS). Dialogul „Review and publish" ținea eroarea veche în cache → bifat „Try to publish items with errors" → Publish → anunțul **Processing** (review Meta, de obicei < 24 h). Campania: 75 lei/zi, InitiateCheckout. Lecție: pe cont NOU Meta cere card VERIFICAT + telefon verificat + pagină confirmată înainte de primul anunț; checklist-ul e în Ads Manager → Account overview → „Get set up to run ads". |
| 03.09 15:40 | **Anunțul C0 RESPINS** de Meta (review automat, ~20 min după publicare): „Fraud, scams and deceptive practices — It looks like your ad promotes stolen or forged goods or services". Clasificatorul de „documente falsificate": imaginea (coperta) listează „Certificat constatator · Cazier judiciar · Acte de stare civilă · Extras carte funciară", textul zice „certificat" + „PDF semnat electronic". Contul NU e restricționat (Business Support Home → Your rejected ads: 1 available for review). Opțiuni: (a) **Request review** cu explicație (intermediar privat; documentul e emis de ONRC, noi doar depunem cererea și livrăm; landing-ul spune „nu suntem instituție"); (b) creativ nou doar cu constatator (C1, fără lista de documente în imagine) + text fără „semnat electronic". Recomandare: (a) + (b) în paralel; nu re-publicăm același anunț neschimbat. |
| 03.09 16:10 | **Contestație trimisă** pentru C0 (Business Support Home → Request review → „I believe my ad follows your policies"; răspuns ≤ 2 zile). **Upload rezolvat prin pagina de Facebook**: www.facebook.com are `<input type=file>` real → am publicat pe pagină P2 (c1), P5 (carusel c2-1..4), P11 (c3), P9 (c4), P1 (lansare platformă, **pinned**), P13 (C6–C8) → toate imaginile apar în Ads Manager la Media → „Page images". **Anunțuri noi publicate din duplicarea lui C0** (bulk edit: nume, primary text, headline, description, URL parameters, media per anunț): `C1 Deadline banca - static` (120252345737280556), `C3 Licitatie fonduri - static` (120252345737290556), `C4 Verificare partener - static` (120252345737300556) — toate **In review**; descrierea schimbată în „Preț final afișat. Taxe ONRC incluse. Livrare pe email." (fără „semnat electronic", care a contribuit la respingerea C0). **Higgsfield**: CLI instalat + skills + login (cont serviciiseonethut, 249 credite); `product-photoshoot` (enhance endpoint) pică cu „Cannot reach", fallback `generate create gpt_image_2` 3:4 2k → 3 scene curate (laptop+email, certificat cu sigiliu auriu, semnare contract + tablet), salvate în `assets/higgsfield/hf-scene1..3.png`; compuse cu bandă de text + preț (PIL) → `assets/c6-hf-deadline-banca.png`, `c7-hf-document-email.png`, `c8-hf-verificare-partener.png` (+ variante 1:1). Video C5 (screencast) rămâne de urcat de Raul (Reels/Stories). |
| 03.09 16:25 | **C1, C3, C4 APROBATE → Active** (review în ~10 min). Livrarea începe; 0 lei cheltuiți la 16:25. C0 rămâne Rejected, contestația „In review" (Business Support Home: In review 1). Concluzie: respingerea a venit de la creativul cu lista de documente + „semnat electronic", nu de la cont/landing. Urmărire de mâine conform `../README.md` „Ce urmărim". |
