...
| 17.09 13:30 | ⚠️ **Corecție la nota de la 12:40: `oppref` NU se poate adăuga manual.** La salvarea câmpului `Tracking parameters` cu `&oppref={oppref}`, Ads Manager respinge cu `Reserved query parameters are not supported: olref, oppref.` Deci `oppref` e parametru **rezervat**, pe care OpenAI îl adaugă singur pe URL-ul de aterizare; macro-ul `{oppref}` din lista de placeholdere e destinat folosirii ca VALOARE sub un nume propriu, nu ca nume de parametru. Concluzia se răstoarnă a doua oară: nu lipsea nimic din configurare. Cele 23 de clicuri aterizează cu UTM și, foarte probabil, și cu `oppref` — pur și simplu n-a început nimeni o comandă, deci n-avem niciun rând de atribuire în care să se vadă. **Ce am făcut totuși:** am adăugat `&oai_ref={oppref}` (nume propriu, acceptat), iar `attribution.ts` citește acum `oppref` sau `oai_ref`. Rostul e diagnostic: dacă un draft viitor are `oai_ref` dar nu `oppref`, știm că adăugarea automată nu ajunge la noi; dacă le are pe amândouă, lanțul e complet. Anunțul a rămas `Serving` după salvare, fără reintrare în review, iar afișările au continuat să crească (1.041 → 1.044 în câteva minute). |
