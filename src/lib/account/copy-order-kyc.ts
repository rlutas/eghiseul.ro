...
import {
  ...
  uploadKycDocument,
  type KycDocumentType, getFileInfo } from '@/lib/aws/s3';
...
export async function copyOrderKycDocumentsToAccount(
  ...
  for (const doc of uploadedDocuments) {
    ...
    // …and the key must be the one `/submit` writes for THIS type
    // (`kyc/<orderId>/<type>.<ext>`): one object declared as both the front
    // and the selfie must not become two account rows (REV3-KYC-001).
    if (doc.s3Key) {
      const typeBound = new RegExp(`^kyc/${orderId}/${doc.type}\\.[a-z0-9]+$`);
      if (!doc.s3Key.startsWith(allowedPrefix) || !typeBound.test(doc.s3Key)) {
        console.warn(`${logPrefix}: ${doc.type} key is not the order's own key for that type, skipped`);
        continue;
      }
      try {
        const info = await getFileInfo(doc.s3Key);
        if (!(info.size > 0)) throw new Error('empty object');
      } catch {
        console.warn(`${logPrefix}: ${doc.type} object missing in S3, skipped`);
        continue;
      }
    }
  ...
  if (copied > 0) {
    ...
    const activeTypes = ((activeRows ?? []) as Array<{ document_type: string; expires_at: string | null }>)
