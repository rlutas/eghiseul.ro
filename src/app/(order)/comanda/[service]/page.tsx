...
export async function generateMetadata({
  ...
  const [data, brand] = await Promise.all([getService(serviceSlug), getBrand()]);

  if (!data) {
    return {
      title: { absolute: `Comandă - ${brand.name}` },
    };
  }

  return {
    title: { absolute: `Comandă ${data.service.name} - ${brand.name}` },
