...
export function ContactStep({ onValidChange }: ContactStepProps) {
  const { state, updateContactData } = useOrderWizard();

  const form = useForm<ContactFormData>({
    resolver: zodResolver(contactSchema),
    defaultValues: {
      email: state.contactData.email || '',
      phone: state.contactData.phone || '+40 ',
      preferred_contact: state.contactData.preferred_contact || 'email',
    },
    mode: 'onChange',
  });
  ...
  useEffect(() => {
    const subscription = form.watch((value) => {
      updateContactData(value as Partial<ContactFormData>);
    });
    return () => subscription.unsubscribe();
  }, [form, updateContactData]);
