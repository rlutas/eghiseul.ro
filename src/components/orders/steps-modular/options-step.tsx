...
export function OptionsStepModular({ onValidChange }: OptionsStepProps) {
  const { state, updateOptions, priceBreakdown, serviceOptions } = useModularWizard();
  ...
  const getSelectedOption = (optionId: string) => {
    return selectedOptions.find((o) => o.optionId === optionId);
  };
  ...
  const toggleOption = (option: ServiceOption) => {
    if (isSelected(option.id)) {
      // Remove option
      updateOptions(selectedOptions.filter((o) => o.optionId !== option.id));
    } else {
      // Add option
      const newOption = {
        optionId: option.id,
      ...
      updateOptions([...selectedOptions, newOption]);
    }
  };
          ...
          <div>
            ...
            {selectedOptions.length === 0 ? (
              <p className="text-sm text-neutral-600">
                ...
                de bază de {priceBreakdown.basePrice} RON.
              </p>
            ) : (
                ...
                {selectedOptions.map((opt) => (
                  ...
                  >
                    ...
                    <span className="font-medium text-secondary-900">
                      +{opt.priceModifier * opt.quantity} RON
                    </span>
                  </div>
                ))}
